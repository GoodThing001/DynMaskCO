...
import formal_gate_contract as _contract
...
def _validate_formal_protocol(frozen_plan, args):
    ...
    # 正式模式下冻结计划的 role 必须与 --mode 一致（VAL 不能拿 dev_gate 冻结计划）。
    # 旧 dev_gate 冻结计划无 role 字段（隐含 dev_gate），允许；VAL 必须显式 role=val。
    if args.mode in ('dev_gate', 'val'):
        role = frozen_plan.get('role')
        if role is not None and role != args.mode:
            _fail(f'frozen role 不符：{role!r} != --mode {args.mode!r}')
        if role is None and args.mode == 'val':
            _fail('--mode val 要求 frozen plan role=val（旧 dev_gate 冻结计划不能用于 VAL）')
...
def _recompute_profile_hash(profile_path):
    """从 profile 内容重算 hash（单一来源：formal_gate_contract）。"""
    return _contract.recompute_profile_hash(profile_path)


def _combined_hash(file_hashes):
    """与 run_dev_gate._version 同规则的多文件组合 hash（单一来源：formal_gate_contract）。"""
    return _contract.combined_hash(file_hashes.items())
...
def main():
    ...
    ap.add_argument('--mode', choices=['regression', 'dev_gate', 'val'], default='regression')
    ...
    if frozen_plan is not None and frozen_plan.get('contract_identity'):
        expected_contract_hash = frozen_plan['contract_identity'].get('effective_contract_hash')
    ...
    if args.mode in ('dev_gate', 'val') and manifest.get('split_role') != args.mode:
        _fail(f'--mode {args.mode} 要求 split_role={args.mode}，实际 {manifest.get("split_role")}')
    if args.mode in ('dev_gate', 'val'):
        if frozen_plan is None:
            _fail(f'--mode {args.mode} 要求 --frozen-plan')
        if args.profile is None:
            _fail(f'--mode {args.mode} 要求 --profile')
        if not frozen_plan.get('cells'):
            _fail(f'--mode {args.mode} 要求 frozen plan 含 cells（冻结 cell/seed 信息）')
    ...
    if args.mode in ('dev_gate', 'val') and m != 128:
        _fail(f'--mode {args.mode} 要求每 cell 128 实例，实际 {m}')
