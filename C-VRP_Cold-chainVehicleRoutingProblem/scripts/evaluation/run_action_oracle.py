...
from coldchain_contract import (default_pilot_contract, default_pilot_profile,
                                ...
                                load_coldchain_contract, ColdChainContract)
...
def _load_contract(args):
    """加载 cold-chain contract（--coldchain-contract）；缺省回退显式 pilot contract。

    正式新 DEV-GATE / VAL 必须显式提供 contract；旧命令仍回退 pilot 保持历史兼容。
    """
    if args.coldchain_contract is None:
        return default_pilot_contract()
    return load_coldchain_contract(args.coldchain_contract)


def _make_env(dataset, capacity, num_vehicles, continuation, objective, profile, contract=None):
...
def _worker_instance(args):
    (inst_idx, data_path, capacity, num_vehicles, objective,
     profile_manifest, contract_manifest, do_local, out_dir, code_sha256) = args
    ...
    try:
        ...
        contract = (ColdChainContract.from_manifest(contract_manifest)
                    if contract_manifest is not None else None)

        # 统一 factory：每条 rollout 分支独立 JF1-H-F continuation（阶段 C 步骤 1，
        # 隔离 deferred 集合与累计审计状态，避免跨分支串扰）
        env = _make_env(dataset, capacity, num_vehicles, make_continuation(), objective,
                        profile, contract)
        ...
        log = []
        oenv = _make_env(dataset, capacity, num_vehicles, make_continuation(), objective,
                         profile, contract)
        renv = _make_env(dataset, capacity, num_vehicles, make_continuation(), objective,
                         profile, contract)
        ...
        local = None
        if do_local:
            local = _local_instance(dataset, capacity, num_vehicles, objective, profile,
                                    inst_idx, contract)
...
def main():
    ...
    parser.add_argument('--coldchain-contract', default=None,
                        help='cold-chain contract JSON（缺省回退显式 pilot contract）')
    ...
    contract_manifest = contract.to_manifest() if args.objective == 'coldchain' else None
    ...
    args_list = [(i, args.data, args.capacity, args.num_vehicles, args.objective,
                  profile_manifest, contract_manifest, args.local, args.out,
                  code_sha256) for i in idxs]
